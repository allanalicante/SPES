 ...
 if(isset($_REQUEST['classid']))
    {
        ...
        $query4 = "SELECT SchoolYear,SchoolHead FROM `schoolyear_tbl` where Active = 'Yes'";
...
else if (isset($_REQUEST['showhistory']))
{
    $thisClass = $_REQUEST['showhistory'];
        $output = '';
        // Query for selecting list of student per section
        $query = "SELECT DISTINCT (s.id) AS `studentID`, s.lrn, s.photo, s.lastname, s.firstname, s.middlename,
                g.grade, ss.sectionname, t.name AS `Advisor`, s.modality, s.studenttype 
                ...
                INNER JOIN teacher t
                ON t.id = ss.teacher_id
                WHERE ss.id = '".$_REQUEST['showhistory']."'
                GROUP BY studentID
                ORDER BY s.lastname ASC";        
                $result = mysqli_query($connection, $query);
                $i=1;
        ...
        $query2 = "SELECT s.teacher_id, CONCAT(g.grade,' - ', s.sectionname) AS `Class`, g.grade, s.sectionname, u.image, t.name
                ...
                INNER JOIN users u
                ON u.id = t.id
                WHERE s.id ='".$_REQUEST['showhistory']."'";
                $result2 = mysqli_query($connection, $query2);
                while ($row = mysqli_fetch_array($result2)) { 
                    $gradelevel = $row['grade'];
                    $section = $row['sectionname'];         
        ...
        // Query for counting total numbers of pupils / boys / girls per section
        $query3 = "SELECT COUNT(IF(s.sex = 'Male' and e.status = 'renew',s.id, NULL)) AS `male`, 
                COUNT(IF(s.sex = 'female' and e.status = 'renew',s.id, NULL)) AS `female`, COUNT(iF(e.status='renew',s.id,NULL)) AS `total`
                FROM student_tbl s
                INNER JOIN enrollment_tbl e
                ON s.id = e.student_id
                WHERE e.status = 'renew'
                AND e.section_id ='".$_REQUEST['showhistory']."'";
                $result3 = mysqli_query($connection, $query3);
                while ($row = mysqli_fetch_array($result3)) {           
                $male = $row['male'];
                $female = $row['female'];
